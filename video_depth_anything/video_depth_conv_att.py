...
class VideoDepthAnything(nn.Module):
    def __init__(
        ...
        if self.diff:
            self.diff_layers = nn.Sequential(
                nn.Conv2d(in_channels=3, out_channels=out_channel//2, kernel_size=3, padding=1),
                nn.BatchNorm2d(out_channel//2),
                nn.ReLU(True),
                nn.Conv2d(in_channels=out_channel//2, out_channels=out_channel, kernel_size=3, padding=1),
                nn.BatchNorm2d(out_channel),
                nn.ReLU(True)
            )
            if self.cross_att:
                self.diff_proj = nn.Conv2d(out_channel, EMB_DIM, kernel_size=1)
                self.fuse_mha = nn.MultiheadAttention(embed_dim=EMB_DIM, num_heads=4)
                #self.fuse_mha = CustomMultiheadAttention(embed_dim=EMB_DIM, num_heads=1)
            else:
                self.mlp = nn.Linear(out_channel, EMB_DIM)
    ...
    def forward(self, x):
        ...
        if self.diff:
            ...
            conv_flat = self.diff_layers(x_flat)
            conv = conv_flat.view(B, T, -1, H, W)
            diff_tensor = conv[:, 1:] - conv[:, :-1]
            diff_flat = diff_tensor.flatten(0, 1)
            ## 이제 나오는 차원은 (B*(T-1), C_updated , H, W)

            pooled = F.adaptive_avg_pool2d(diff_flat, (patch_h, patch_w))
            ## patch는 

            if self.cross_att:
                pooled = self.diff_proj(pooled)
            else:
            ...
            for i in range(T):
                if i == 0:
                    ...
                    diff_per_frame.append(pooled[:, -1])
                else:
                    avg_ij = 0.5 * (pooled[:, i-1] + pooled[:, i])
                    diff_per_frame.append(avg_ij)
            diff_all = torch.stack(diff_per_frame, dim=1)
            diff_result = diff_all.permute(0,1,3,4,2).view(B*T, patch_h*patch_w, EMB_DIM)

            fused_features = []
            for (patch_tokens, cls_token) in features:
                Q = patch_tokens.transpose(0,1)
                K = diff_result.transpose(0,1)
                V = diff_result.transpose(0,1)
                attn_out, _ = self.fuse_mha(Q, K, V)
                patch_fused = attn_out.transpose(0,1)
                fused_features.append((patch_fused, cls_token))

            depth = self.head(fused_features, patch_h, patch_w, T)
        else:
